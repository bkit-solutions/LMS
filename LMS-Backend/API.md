...
Below is a complete, copy-paste friendly flow with example requests for each feature. Use your actual host/port, e.g., `http://localhost:8080`.
...
SUPER_TOKEN=$(curl -s -X POST http://localhost:8080/api/auth/login \
  ...
  -d '{"email":"super@lms.com","password":"superpass"}' \
  | jq -r '.data.token')
```
...
- Answer correctness:
    ...
    - MAQ: `answerText` is CSV like `"A,B,D"`; compared as sets (order-insensitive, case-insensitive).
    - FILL_BLANK: compared after lowercasing, trimming, removing spaces/dashes/underscores.
- Scoring: uses `marks` (default 1) and `negativeMarks` (default 0). Total floored at 0.
- Ownership: Admin can manage only their own tests/questions; Superadmin bypasses ownership checks.
- Auth header is required for all non-`/api/auth/**` routes.
